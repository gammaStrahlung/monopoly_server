...
if (auction.getBids().size() < auction.getExpectedBids()) {
    ...
    Bid resultBid = auction.evaluateHighestBids();
    String jsonData = gson.toJson(resultBid);

    return ServerMessage.builder().messagePath("bidResult").type(ServerMessage.MessageType.INFO).jsonData(jsonData).build();
}
    }
    ...
    private static ServerMessage checkCurrentField(ClientMessage clientMessage) {
        ...
        try {
            ...
            return ServerMessage.builder().messagePath("checkCurrentField").type(ServerMessage.MessageType.ERROR).build();
        }

        boolean isProperty = auction.checkCurrentField(currentFieldIndex);
        if (isProperty) {
            return  ServerMessage.builder().messagePath("checkCurrentField").type(ServerMessage.MessageType.INFO).jsonData("true").build();
        } else {
    ...
    private static ServerMessage startAuction() {
        return ServerMessage.builder().messagePath("startAuction").type(ServerMessage.MessageType.INFO).build();
    }
